...
def load_card_list(generator_config):
    ...
    result["landscapes_queries"] = result["landscapes_regular"].groupby(["Type"])['Count'].sum().reset_index().set_index('Type')
    ...
    for k, df in result.items():
        ...
        exported_df.to_excel(Path.cwd().parent / "config" / f"{k}.xlsx", index=(isinstance(exported_df.index, MultiIndex) or exported_df.index.name is not None))

    return result


template_names = {
    "card_amount_tables.hpp": Path("config"),
    "amount_getters.hpp": Path("config"),
    "amount_getters.cpp": Path("config"),
    "component.config.generator_bounds.cpp": Path("tst")
}


def load_header_templates():
    return dict((template_name, open(Path.cwd() / "templates" / f"{template_name}.mustache").read()) for template_name in template_names.keys())


def render_config_files(template, template_vars):
    chdir(Path.cwd() / "templates")
    for template_name, template_path in template_names.items():
        with open(Path.cwd().parent.parent / template_path / f"{template_name}", "w") as f:
